 ...
 *  now the heading is a line in it that can be written in. So the page is put
 *  right first: each list without a heading gets one saying what it always
 *  said, drawn as the Title it always was.
 *
 *  And a list with more than one keeps only its first. That is two devices
 *  each giving the same old list its heading while apart, then merging. The
 *  editor deletes any node that doesn't fit its schema, from the document and
 *  so from every device, and a list with two headings doesn't.
 *
