...
supabase/functions/       Edge Functions (Deno): the sign-up alert email
...
### Sign-up alerts (optional)

Jottr can email you each time someone confirms a new account, so you notice
if it starts getting traction. A trigger on `auth.users` in `schema.sql` calls
the [`notify-signup`](supabase/functions/notify-signup/index.ts) Edge Function,
which sends the email over SMTP. Until the two Vault secrets below exist the
trigger does nothing, and a failure anywhere in it never stops anyone signing
in.

1. **Pick a shared secret**, any long random string (`openssl rand -hex 32`).
2. **Store two secrets in Vault.** In the SQL Editor, run this once (re-running
   it errors, because the names already exist):

   ```sql
   select vault.create_secret('https://<project-ref>.supabase.co', 'project_url');
   select vault.create_secret('<shared secret>', 'signup_alert_secret');
   ```

3. **Deploy the function** with JWT verification off. The shared secret stands
   in for it:

   ```bash
   npx supabase login
   npx supabase functions deploy notify-signup --no-verify-jwt --project-ref <project-ref>
   ```

4. **Set the function's secrets.** Use the same SMTP account as the Auth
   emails (Authentication → Emails → SMTP Settings), but on port 465: Edge
   Functions can't connect out on 25 or 587.

   ```bash
   npx supabase secrets set --project-ref <project-ref> \
     SIGNUP_ALERT_SECRET=<shared secret> \
     SIGNUP_ALERT_TO=you@example.com \
     SMTP_HOST=smtp.example.com SMTP_PORT=465 \
     SMTP_USER=... SMTP_PASS=... SMTP_FROM="Jottr <noreply@example.com>"
   ```

   `SMTP_FROM` is optional and falls back to `SMTP_USER`. Sign-ups from
   `SIGNUP_ALERT_TO` itself are skipped.
5. **Re-run `schema.sql`** if it predates the trigger.

To test, sign up with an address you haven't used before. If no email comes,
check the function's logs (Edge Functions → notify-signup → Logs), then the
`net._http_response` table, which records each request the trigger made.
